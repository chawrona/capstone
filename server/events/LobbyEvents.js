...
export default class LobbyEvents {
  ...
  onCreateLobby({ userId }) {
    try {
      ...
      this.logger.log(`Stworzono pokój o id ${lobbyId} przez gracza ${userId}`);
    } catch (error) {
  ...
  onGameStart({ userId }) {
    try {
      ...
      for (const user of users) {
        ...
        if (colorsInUse.has(user.color)) throw new ColorDuplicatedError();
        colorsInUse.add(user.color);
      }

      if (users.some((user) => !user.isReady && !lobby.isAdmin(user.id))) {
        // => to return
