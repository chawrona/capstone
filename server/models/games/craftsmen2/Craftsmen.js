...
import Craftsman2 from "./Craftsman.js";

export default class Craftsmen2 extends Game {
    constructor(players, endGame, lobbyId, playerClass) {
        ...
        */
        ...
        // region Config
        this.maxPlayersInventorySpace = 10;
        this.rotateCost = {
            resources: [["iron", 1]],
            coins: 2,
        };
    ...
    initialSetup() {
        ...
        for (const [publicId, player] of this.players) {
            ...
            const craftsman = new Craftsman2(
                publicId,
                player.color,
                craftsmanId,
            );
    ...
    drawSingleContract(index) {
        if (this.contracts[0].length === 0) {
            if (this.contracts[1].length > 0) {
                this.contracts[0].push(this.contracts[1].shift());
            } else if (this.contracts[2].length > 0) {
                this.contracts[0].push(this.contracts[2].shift());
            } else {
                throw new Error("Nie ma już kart kontraktów");
            }
        }

        this.currentContracts[index] = this.contracts[0].shift();
    }
    ...
    buyCraftsman({ publicId }) {
        ...
        const craftsman = new Craftsman2(publicId, player.color, craftsmanId);
        this.craftsmen.set(craftsmanId, craftsman);
        this.craftsmenOwners.get(publicId).add(craftsmanId);
    ...
    rerollContract({ publicId, data: contractId }) {
        ...
        // Wymieniamy wszystkie 3 kontrakty
        for (let i = 0; i < this.currentContracts.length; i++) {
            this.reroll(i);
        }
