...
export default class Ludo extends Game {
    ...
    for (const [, player] of this.players) {
      targets.push({
        ...
        data: {
          ...this.gameData,
          yourTurn:
            player.publicId === this.playersQueue[this.currentPlayerIndex],
  ...
  _dataWithPlayerTarget(publicId) {
    ...
    return {
      ...
      data: {
        ...this.gameData,
        yourTurn: publicId === this.playersQueue[this.currentPlayerIndex],
        currentPlayerIndex: this.currentPlayerIndex,
        possiblePawnMoves,
    ...
    this.gameData.gameMap.forEach((pawn, currentPosition) => {
      if (!Number.isInteger(pawn) && pawn[0] === currentPlayerPublicId) {
        ...
        if (lapDone) {
          ...
          let finishPosition = newPosition - currentPlayerMapStartPoint + 1;
      ...
      if (lapDone) {
        ...
        for (const playerPawns of this.gameData.finishPositions) {
          if (playerPawns[0][0] === currentPlayerPublicId) {
            ...
            for (const playerPawns of this.gameData.finishPositions) {
              if (playerPawns[0][0] === currentPlayerPublicId) {
                playerPawns.push([currentPawn[0], finishPosition]);
                break;
              }
            }
        ...
        if (!pawnOnNewPosition) {
          this.gameData.gameMap[newPosition] = this._copyPawn(currentPawn);
          this.gameData.gameMap[currentPosition] = 0;
